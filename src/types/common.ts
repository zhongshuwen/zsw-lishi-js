...
/**
 * A `int64_t` natively in `zswchain` but can become a string when > 32 bits number
 * due to how `zswchain` serialize number to JSON.
 *
 * This is like because JavaScript largest number possible is 53 bits large which
 * make it impossible to hold a full `int64_t` type. To overcome that, `zswchain`
 * will output a string when number is too large to preserve precision.
 */
export type Int64 = number | string

/**
 * A `uint64_t` natively in `zswchain` but can become a string when > 32 bits number
 * due to how `zswchain` serialize number to JSON.
 *
 * This is like because JavaScript largest number possible is 53 bits large which
 * make it impossible to hold a full `uint64_t` type. To overcome that, `zswchain`
 * will output a string when number is too large to preserve precision.
 */
...
export type DbOp<T = unknown> = {
  /**
   * This is a quirk of 中数文历史 API, it's recommended to do a lower case comparison on the `op` field.
   */
