...
/**
 * An interface used to interact with 中数文历史 REST API.
 *
...
export interface HttpClient {
  ...
  /**
   * Make an anonymous request (unauthenticated) to the 中数文历史 Authentication endpoint.
  ...
  /**
   * Make an authenticated request (unauthenticated) to the 中数文历史 REST endpoint.
   *
   * Upon a succesfull call, the actual response body (expected to be a valid JSON) will
   * be returned to the caller.
   *
   * Upon an error, a [[ZswLishiError]] is returned, will be a [[ZswLishiApiError]] if the response's
   * body exist, it's a valid JSON string and it fits the 中数文历史 error format.
   *
