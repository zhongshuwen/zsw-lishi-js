...
import { ZswLishiClient, RequestIdGenerator, ZswLishiClientEndpoints } from "../types/client"
...
import { ZswLishiClientError, ZswLishiError } from "../types/error"
...
/**
 * All the options that can be pass to zswlishi Client factory
 * [[createZswLishiClient]].
 */
export interface ZswLishiClientOptions {
  /**
   * The network to connect to. Can be a plain string in the set
   * `mainnet | testnet`. If it's not a string in this
   * set, the value is assumed to be an hostname pointing to the
   * the service, for example, your internal zswlishi endpoint.
   * 
   ...
   * the [[ZswLishiClientOptions.secure]] option to determine which
  ...
  /**
   * You 中数文历史 API key to interact with the zswlishi API service.
   *
   * If you are connecting to a `zswlishi` instance
   * (or a local instance of our other supported chains) 
   */
  apiKey?: string
   ...
   * the actual hostname as defined by the [[ZswLishiClientOptions.network]]
   * value.
   *
   * @default true
   */
   ...
   *
   * @default 'https://explorer.testnet.' if `authentication: true`, 'null://' otherwise
   */
  authUrl?: string

  /**
   * A function that generates a random request ID. This request ID
   * is used when using the zswlishi Stream API when no specific
   * ID is passed at registration time.
   *
   * @default A generator that generates random id of the form `zl-<13-hex-chars>`.
   */
  requestIdGenerator?: RequestIdGenerator

  /**
   * The [[HttpClient]] instance that [[ZswLishiClient]] should use to interact
   * with zswlishi REST API. When `undefined` (the default), an instance is
   * created using [[createHttpClient]] factory method and used. If
   * [[ZswLishiClientOptions.httpClientOptions]] is set, it used when creating
   * the default instanve.
   *
   * @default A default [[HttpClient]] instance (via [[createHttpClient]]) using [[ZswLishiClientOptions.httpClientOptions]].
   */
   ...
   *
   * This parameter has no effect at all if the [[ZswLishiClientOptions.httpClient]] is
   * provided.
   *
  ...
  /**
   * The [[StreamClient]] instance that [[ZswLishiClient]] should use to interact
   * with zswlishi Stream API. When `undefined` (the default), an instance is
   * created using [[createStreamClient]] factory method and used. If
   * [[ZswLishiClientOptions.streamClientOptions]] is set, it used when creating
   * the default instanve.
   *
   * @default A default [[StreamClient]] instance (via [[createStreamClient]]) using [[ZswLishiClientOptions.streamClientOptions]].
   */
   ...
   *
   * This parameter has no effect at all if the [[ZswLishiClientOptions.streamClient]] is
   * provided.
   *
  ...
  /**
   * The [[GraphqlStreamClient]] instance that [[ZswLishiClient]] should use to interact
   * with zswlishi GraphQL Subscription API. When `undefined` (the default), an instance is
   * created using [[createGraphqlStreamClient]] factory method and used. If
   * [[ZswLishiClientOptions.graphqlStreamClientOptions]] is set, it used when creating
   * the default instanve.
   *
   * @default A default [[GraphqlStreamClient]] instance (via [[createGraphqlStreamClient]]) using [[ZswLishiClientOptions.graphqlStreamClientOptions]].
   */
   ...
   *
   * This parameter has no effect at all if the [[ZswLishiClientOptions.graphqlStreamClient]] is
   * provided.
   *
  ...
  /**
   * The API token store instance that should be use by the [[ZswLishiClient]]
   ...
   * refresh. This is more an internal details of the [[ZswLishiClient]] should
...
/**
 * The main entry point of the library, use it to create the standard [[ZswLishiClient]]
 * instance.
 *
 ...
 *
 * @param options The options that can be passed to customize [[ZswLishiClient]] instance,
 * refer to the [[ZswLishiClientOptions]] for further details.
 *
 * @kind Factories
 */
export function createZswLishiClient(options: ZswLishiClientOptions): ZswLishiClient {
  ...
  let defaultAuthUrl = "https://lishi.zhongshuwen.com"
  ...
  const endpoints: ZswLishiClientEndpoints = {
    authUrl,
...
function checkApiKey(apiKey: string | undefined, authentication: boolean | undefined): void {
  ...
  if (apiKey == null) {
    ...
    throw new ZswLishiError(messages.join("\n"))
  }
...
function inferApiTokenStore(apiKey: string | undefined): ApiTokenStore {
  const debug = debugFactory("zswlishi:client")
  if (!apiKey) {
  ...
  if (typeof window !== "undefined" && window.localStorage != null) {
    ...
    return new LocalStorageApiTokenStore("zswlishi:token")
  }
...
export function networkToEndpoint(network: string): string {
  if (network === "testnet") {
    return "explorer.testnet.chao7.cn"
  } else if (network === "mainnet") {
    return "lishi.zhongshuwen.com"
  } else {
    return network;
  }
}
...
/**
 * The standard implementation of a [[ZswLishiClient]].
 *
 ...
 * methods on it (other ZSWCC endpoints).
 */
export class DefaultClient implements ZswLishiClient {
  public readonly endpoints: ZswLishiClientEndpoints
  ...
  protected debug: IDebugger = debugFactory("zswlishi:client")

  constructor(
    id: number,
    apiKey: string | undefined,
    endpoints: ZswLishiClientEndpoints,
    httpClient: HttpClient,
    streamClient: StreamClient,
    graphqlStreamClient: GraphqlStreamClient,
    apiTokenStore: ApiTokenStore,
  ...
  public release(): void {
    this.debug("Releasing default zswlishi client")
  ...
  // not a proper implementation of `ZswLishiClient.graphql` which has two signatures, both of them
  ...
  private async withApiToken<R>(
    ...
    if (this.apiKey) {
      try {
        ...
        throw new ZswLishiClientError("Unable to obtain the API token", error)
      }
    }
...
function randomReqId(): string {
  return `zl-${Math.random().toString(16).substr(2)}`
}
