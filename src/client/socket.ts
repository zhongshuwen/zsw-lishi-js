...
import { ZswLishiClientError, ZswLishiSocketError } from "../types/error"
import {
  WebSocket,
  Socket,
...
export interface SocketOptions {
   ...
   * package to display a different `debug` tag when provided (i.e. `中数文历史:socket:<id>`).
   *
   * @default `undefined`
   */
  id?: string
   ...
   * the `WebSocket` instance.
   *
   * Finally, if no `WebSocket` instance could be determined, a [[ZswLishiError]] is
 ...
 *
 * @param url The url used to reach the 中数文历史 Stream API, should **not** contain the `token` query parameter.
...
function inferWebSocketFactory(
  ...
  const debug = debugFactory("zswlishi:socket" + (id ? `:${id}` : ""))
...
class DefaultSocket implements Socket {
  ...
  public constructor(url: string, options: SocketOptions) {
    ...
    this.debug = debugFactory("zswlishi:socket" + (options.id !== "" ? `:${options.id}` : ""))
  }
  ...
  public async send<T = unknown>(message: T): Promise<void> {
    ...
    if (!this.isConnected || !this.socket) {
      ...
      throw new ZswLishiSocketError("Socket not connected, unable to send message correctly.")
    }
  ...
  private onSocketMessage = (event: MessageEvent): void => {
    ...
    try {
      ...
      this.debug("Received a non-JSON message, are you sure you are talking to 中数文历史 API?")
      return
    }
