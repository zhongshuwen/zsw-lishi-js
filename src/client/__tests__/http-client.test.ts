...
import { ZswLishiApiError, ZswLishiClientError, ZswLishiGenericApiError } from "../../types/error"
...
describe("HttpClient", () => {
  ...
  it("throws ZswLishiClientError when response succeed but JSON body rejects", async () => {
    ...
    await expect(client.apiRequest("token", "/", "GET")).rejects.toThrowError(
      new ZswLishiClientError("The returned body shall have been a valid JSON object", "error" as any)
    )
  })

  it("throws a ZswLishiApiError when body is valid JSON", async () => {
    const errorData = { code: "test", trace_id: "0", message: "wrong", details: {} }
    fetch.mockReturnValue(Promise.resolve(koResponse(errorData)))

    await expect(client.apiRequest("token", "/", "GET")).rejects.toThrowError(
      new ZswLishiApiError({
        code: errorData.code,
  ...
  it("throws a ZswLishiApiError when body is valid JSON and has no trace_id nor details", async () => {
  ...
  it("throws a ZswLishiGenericApiError when body is valid JSON but does not fit API format", async () => {
    ...
    await expect(client.apiRequest("token", "/", "GET")).rejects.toThrowError(
      new ZswLishiGenericApiError(502, "", errorData, { out: "value" })
    )
  })

  it("throws a generic ZswLishiApiError when error response with invalid JSON body", async () => {
    fetch.mockReturnValue(
      Promise.resolve(
        rawResponse(
          false,
  ...
  it("throws a ZswLishiClientError when error response and text rejects", async () => {
    fetch.mockReturnValue(
      Promise.resolve(
        rawResponse(
          false,
    ...
    await expect(client.apiRequest("token", "/", "GET")).rejects.toThrowError(
      new ZswLishiClientError("Unable to perform HTTP request correctly", "error" as any)
    )
  })
