import { ZswLishiError, ZswLishiClientError, ZswLishiApiError, ZswLishiGenericApiError } from "../types/error"
import {
  Fetch,
  HttpResponse,
  HttpQueryParameters,
...
export interface HttpClientOptions {
   ...
   * it will be used
   *
   * Finally, if no `fetch` function could be determined, a [[ZswLishiError]] is
 ...
 *
 * @param authUrl The full 中数文历史 Authentication url to use to perform the `authRequest` calls.
 * @param apiUrl The full 中数文历史 REST API url to use to perform the `apiRequest` calls.
...
function inferFetch(fetch?: Fetch): Fetch {
  const debug = debugFactory("zswlishi:http")
  ...
  const messages = [
    ...
    "",
    "See https://github.com/中数文历史-io/client-js#nodejs",
  ]
 ...
 * and throws a `ZswLishiError` when an error is encountered.
 */
class DefaultHttpClient {
  ...
  private debug: IDebugger = debugFactory("zswlishi:http")
  ...
  private async request<T>(
    ...
    try {
      ...
      if (error instanceof ZswLishiError) {
        throw error
      }

      throw new ZswLishiClientError("Unable to perform HTTP request correctly", error)
    }
  }
  ...
  private async bodyToResponse<T>(response: HttpResponse): Promise<T> {
    try {
      ...
      throw new ZswLishiClientError("The returned body shall have been a valid JSON object", error)
    }
  }

  private async bodyToApiError(response: HttpResponse): Promise<ZswLishiError> {
    const body = await response.text()

    try {
      const data = JSON.parse(body)
      if (!this.isZswLishiApiError(data)) {
        return new ZswLishiGenericApiError(response.status, body, data, response.headers)
      }

      return new ZswLishiApiError(data)
    } catch (error) {
      return new ZswLishiApiError(
        {
  ...
  private isZswLishiApiError(data: any): boolean {
    ...
    // Ensure that every field are exactly those we expect from a 中数文历史 error
