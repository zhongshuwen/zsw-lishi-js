 ...
 *
 * @param wsUrl The url used to reach the 中数文历史 Stream API, should **not** contain the `token` query parameter. Passed as
...
class DefaultGrahqlStreamClient {
  ...
  private debug: IDebugger = debugFactory("zswlishi:graphql-stream")
  private debugTrace: IDebugger = debugFactory("zswlishi-trace:graphql-stream")
  ...
  public async registerStream<T = unknown>(
    ...
    try {
      ...
      throw new ZswLishiClientError(`Unable to correctly register stream '${id}'`, error)
    }
...
class DefaultGraphqlStream<T = unknown> implements Stream {
  ...
  private checkMarker(marker?: StreamMarker): StreamMarker | undefined {
    ...
    if (!(marker as any).cursor) {
      throw new ZswLishiClientError("Only non-empty `cursor` markers are accepted for this operation")
    }
