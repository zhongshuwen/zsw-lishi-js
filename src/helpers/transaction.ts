 ...
 * a single flat list. The flat list indexes matches 中数文历史 API action index
 * which is used within 中数文历史 to identify particular action trace in
 ...
 *   ExecutionTrace {
 *     ActionTraces: [
 *       ActionTrace(zswhq.token/zswhq.token:transfer) {
 *         InlineTraces: [
 *           ActionTrace(from/zswhq.token:transfer) {
 *             InlineTraces: [
 *               ActionTrace(contractX/contractX:log)
 *             ]
 *           }
 *           ActionTrace(to/zswhq.token:transfer) {
 *             InlineTraces: [
 ...
 * index of the list represents the action's index concept of 中数文历史 API.
 */
