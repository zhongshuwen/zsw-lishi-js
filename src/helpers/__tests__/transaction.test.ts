...
describe("flattenActions", () => {
  ...
  it("flattens action traces correctly", () => {
    const transaction = createTransaction([
      createActionTrace("zswhq.token/zswhq.token:transfer", [
        createActionTrace("from/zswhq.token:transfer", [
          createActionTrace("contractX/contractX:log")
        ]),
        createActionTrace("to/zswhq.token:transfer", [
    ...
    expect(flattenActionTraces(transaction).map(toTriplet)).toEqual([
      "zswhq.token/zswhq.token:transfer",
      "from/zswhq.token:transfer",
      "contractX/contractX:log",
      "to/zswhq.token:transfer",
